...
# 检测模型的类别
# class_names=["red_stop","green_go","speed_unlimited","yellow_back","pedestrian_crossing","speed_limited"]#extreme 1-2
# class_names = ["yellow_back", "speed_unlimited", "green_go", "speed_limited", "red_stop",
#                "pedestrian_crossing"]  # extrem 1-1

class_names = ["speed_limited","speed_unlimited"]  # extrem 1-1
# class_names = ["red_stop", "yellow_back", "green_go"]
# class_names = ["yellow_back", "speed_unlimited", "green_go", "speed_limited", "red_stop"]
...
def draw_boxes(img_data, bboxes):
    ...
    if len(bboxes) == 0: return img_data, "", None, None 
    ...
    cv2.putText(img_data, str(x_max) + '  ' + str(y_max), (x_min, y_max + 20), text_font, font_scale, (0, 255, 0),
                thickness)
    cv2.putText(img_data, str(x_min) + '  ' + str(y_min), (x_min, y_max - 100), text_font, font_scale, (0, 255, 0),
                thickness)
    labelName = class_names[label]
    x1 = (x_min, y_min)
    x2 = (x_max, y_max)
    return img_data, labelName, x1, x2
